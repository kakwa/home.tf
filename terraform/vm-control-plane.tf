...
# Boot disk per control plane VM (backing store = talos_base, grows as needed).
      ...
      var.enable_cloudinit ? [
        {
          ...
          source = {
            volume = {
              pool   = libvirt_volume.cp_seed_volume[each.key].pool
              volume = libvirt_volume.cp_seed_volume[each.key].name
            }
          }
    ...
    graphics = [
      {
        spice = {
          autoport = "no"
          port     = var.vm_spice_port_base + index(sort(keys(local.control_plane_nodes)), each.key)
